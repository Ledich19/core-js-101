...
function timeSpanToString(startDate, endDate) {
  ...
  const milliseconds = addTwoZero(time.getUTCMilliseconds());
...
function angleBetweenClockHands(date) {
  ...
  const hoursDeg = hours * 30 + (30 / 60) * minutes;
  const corner = Math.abs(minutesDeg - hoursDeg);
  const angle = corner > 180 ? 360 - corner : corner;
